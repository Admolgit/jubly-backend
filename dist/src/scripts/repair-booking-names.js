...
async function main() {
    ...
    const serviceNameById = new Map(servicesResult.cursor.firstBatch.map((s) => [s._id.$oid, s.name]));
    ...
    if (unmatched.length) {
        console.warn(`Skipping ${unmatched.length} booking(s) with no matching service: ${unmatched
            .map((b) => b._id.$oid)
            .join(', ')}`);
    }
    if (!repairs.length) {
    ...
    for (const repair of repairs) {
        console.log(`  ${repair.id} (${repair.clientName}): "${repair.currentName}" -> "${repair.serviceName}"`);
    }
    if (!args.apply) {
        console.log('Dry run only. Re-run with "npm run repair:booking-names -- --apply" to persist changes.');
        return;
    }
    for (const repair of repairs) {
